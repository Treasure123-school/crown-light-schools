...
export default function StudentMessages() {
  ...
  const { data: teachers = [] } = useQuery({
    queryKey: ['teachers'],
    queryFn: async () => {
      const response = await fetch(getApiUrl('/api/users?role=Teacher'), {
        credentials: 'include'
      });
            ...
            {isLoading ? (
              ...
              <div className="space-y-3">
                {filteredMessages.map((message: any) => (
                  <Card
                    key={message.id}
                    className={`cursor-pointer transition-colors hover:bg-muted/50 ${selectedMessage?.id === message.id ? 'border-primary' : ''
